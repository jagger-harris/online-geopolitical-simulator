...
class CountryNode {
  constructor(country, data, amount, point) {
    ...
    this.activeMilitary = Math.ceil(Number(data.activeMilitary) / amount);
    this.reserveMilitary = Math.ceil(Number(data.reserveMilitary) / amount);
    this.fertilityRate = Number(data.fertilityRate);
    this.mortalityMaleAdults = Number(data.mortalityMaleAdults);
    this.mortalityFemaleAdults = Number(data.mortalityFemaleAdults);
    this.mortalityInfants = Number(data.mortalityInfants);
    this.lifespan = Number(data.lifespan);
    this.nuclearWeapons = Math.ceil(Number(data.nuclearWeapons) / amount);
    this.selected = false;
    this.captured = false;
    this.borderVertices = [
